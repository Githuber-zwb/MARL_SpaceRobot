...
def main(args):
        ...
        for eval_step in range(all_args.episode_length):
            ...
            if eval_step < 70:
                ...
                for agent_id in range(2):
                    ...
                    # print("step: ",eval_step,"action: ",eval_action)
                    action.append(eval_action)
                for agent_id in range(2,6):
                    actor = actors2[agent_id]
                    actor.eval()
                    # print(actor.act.action_out.logstd._bias)
                    # print("observation: ",np.array(list(obs[agent_id,:])).reshape(1,25))
                    obses = [ob2,ob3,ob4,ob5]
                    eval_action,_,rnn_states_actor = actor(
                        obses[agent_id-2].reshape(1,31),
                        eval_rnn_states,
                        eval_masks,
                        deterministic=True,
                    )
    ...
    # print(error)
    imageio.mimsave(
        str(parent_dir) + "/render/mixed_goal/render_mix_dual_stage.mp4",
        frames,
        # duration=0.01,
    )
