...
def main(args):
    ...
    frames = []
    total_epi = 50
    error_dist = np.zeros(total_epi)
    x = np.linspace(0, 19.9, 200)
    y = np.zeros(200)
    dist_time = [i for i in range(75, 80)]
    ...
    with torch.no_grad():
        ...
        for total_step in range(total_epi):
            print(total_step)
            obs = env.reset()
            ...
            # print("reward: ",eval_rewards)
            error_dist[total_step] = error_i
            print("episode reward: ",np.array(eval_episode_rewards).sum(), "error", error_i)
    ...
    np.save('fig_plot/error_base_rot_mass400.npy', error_dist)
    print("error:",error_dist)
    # imageio.mimsave(
    #     str(parent_dir) + "/render/base_reorien/render_base_rot_fail.mp4",
    #     frames,
    #     # duration=0.01,
    # )
